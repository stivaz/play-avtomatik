...
function get_sticky_option( $option = null ) {
...
function is_stickied() {
	global $post;
	if( intval( $post->sticky_status ) === 1 ) {
		return true;
	} else {
		return false;
	}
...
function is_announcement() {
	global $post;
	if( intval( $post->sticky_status ) === 2 ) {
		return true;
	} else {
		return false;
	}
...
function announcement_banner( $display = true ) {
	...
	if( intval( $post->sticky_status ) === 2 && intval( get_sticky_option( 'display_date' ) ) == 0 ) {
...
function sticky_the_title( $content, $id = null ) {
	global $post;
	if ( $id === $post->ID && intval( $post->sticky_status ) > 0 ) {
		$sticky_options = get_sticky_option();
		$display_before_title = isset( $sticky_options['display_before_title'] ) ? intval( $sticky_options['display_before_title'] ) : 1;
		if( $display_before_title === 1 ) {
...
function sticky_the_content( $content ) {
	global $post;
	switch( intval( $post->sticky_status ) ) {
		case 1:
...
### Function: Add Meta Box To WP Admin
...
?>
	<label for="post_status_announcement" class="selectit"><input type="radio" id="post_status_announcement" name="post_status_sticky" value="2"<?php checked( $post_status_sticky_status, 2 ); ?>/>&nbsp;<?php _e( 'Закреплено', 'wp-sticky' ); ?></label>
	<br />
	<label for="post_status_sticky" class="selectit"><input type="radio" id="post_status_sticky" name="post_status_sticky" value="1"<?php checked( $post_status_sticky_status, 1 ); ?>/>&nbsp;<?php _e( 'В порядке очереди', 'wp-sticky' ); ?></label>
	<br />
	<label for="post_status_normal" class="selectit"><input type="radio" id="post_status_normal" name="post_status_sticky" value="0"<?php checked( $post_status_sticky_status, 0 ); ?>/>&nbsp;<?php _e( 'Обычный', 'wp-sticky' ); ?></label>
<?php
}
...
function sticky_add_meta_box() {
	add_meta_box( 'poststickystatusdiv', __( 'Закрепление в верху списка', 'wp-sticky' ), 'sticky_metabox_admin', array('post', 'igrovue-avtomatu'), 'side' );
}
